...
public class WeiboHomeService {
			...
			for (Status status : statuses) {
				...
				if (status.getRetweeted_Status() != null) {
					values.put("wb_subcontent", status.getRetweeted_Status()
							.getText());
					values.put("wb_subcontent_subpic", status
							.getRetweeted_Status().getThumbnail_pic());
					values.put("wb_submiddle_subpic", status
				...
				db.insert(WeiboApplication.mCurrentUserInfo
						.getScreen_name() + "_home_table", null, values);
			}
	...
	public void deleteHomeInfo() {
		SQLiteDatabase db = dbHelper.getWritableDatabase();
		db.delete(WeiboApplication.mCurrentUserInfo
				.getScreen_name() + "_home_table", null, null);
	}
	...
	public List<Status> selectHomeInfo() {
		String[] columns = { "id", "mid", "portrait", "username", "wb_time",
				"wb_content", "wb_content_pic", "wb_middle_pic", "wb_from",
				"wb_redirect", "wb_comment", "wb_attitude", "wb_subcontent",
				"wb_subcontent_subpic", "wb_submiddle_subpic", "wb_subfrom",
				"wb_subredirect", "wb_subcomment", "wb_subattitude",
				"verified", "verified_type" };
		SQLiteDatabase db = dbHelper.getReadableDatabase();
		List<Status> statuses = null;
		Cursor cursor = db.query(WeiboApplication.mCurrentUserInfo
				.getScreen_name() + "_home_table", columns, null, null,
				null, null, null);
		if (cursor != null && cursor.getCount() > 0) {
			...
			while (cursor.moveToNext()) {
				...
				status.setMid(cursor.getString(cursor
						.getColumnIndex(columns[1])));
				User user = new User();
				user.setProfile_image_url(cursor.getString(cursor
				...
				user.setVerified_type(cursor.getInt(cursor
						.getColumnIndex("verified_type")));
				status.setUser(user);
				...
				status.setBmiddle_pic(cursor.getString(cursor
				...
				retweeted_Status.setBmiddle_pic(cursor.getString(cursor
